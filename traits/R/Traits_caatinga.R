...
madeira$plot <- as.factor(madeira$plot)

                                                  # Tabela com réplicas #
          ####################################################################################################
          #Calculo da densplotade da madeira, capacplotade de armazenamento e conteúdo de água e largura da casca#
          ####################################################################################################

#Densplotade = massa seca/volume fresco# [g/cm3]
#Madeira
...
#Water storaged percentage= porcentagem de água da capacplotade de armazenamento (o quanto de água estava armazenado na seca)
#Madeira
...
copa<- arrange(copa, spnurse)

#Alinhando os plot e nurses
ord <- match (madeira$plot, copa$plot)
copa <- copa[ord,]
#Corfirmação de que deu certo o alinhamento
#madeira$plot==copa$plot
#madeira$spnurse==copa$spnurse
...
traits.mean <- summarise(group_by(traits,spnurse),
...
print ("madeira= traits de madeira por réplica |cop=traits de copa com 5 rplicas |copa = traits de copa por 3 réplica |traits = copa e madeira por réplica |traits.mean = media de todos traits por spnurse")
