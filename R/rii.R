library(reshape2)
library (dplyr)
brutos <- read.csv("data_exp/dados_brutos_experimento.csv", h=T )
head(brutos)
...
alt.rii.mean <- summarise(group_by(alt_medt, espnurse,target),
                          alt.rii.m=mean(alt_medt),
                          alt.rii.sd=sd(alt_medt))
